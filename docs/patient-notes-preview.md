...
The contribution form is active. Typed notes use the site's existing FormSubmit AJAX pattern. Processed handwritten images use FormSubmit's documented native multipart form and return to the same Patient Notes page before showing confirmation. Drafts and selected images remain in memory only; no local storage is used and no submission changes the public registry automatically.
...
Photo submissions use the same provider's documented native multipart form POST with `attachment` and a return page. No new backend, package or service is required. Email field labels are written for moderation rather than exposing registry implementation fields. Typed and photo submissions go to the existing clinic inbox for manual moderation; nothing publishes automatically.
...
The earlier AJAX photo test delivered the fields but not the attachment. The implementation now follows FormSubmit's native multipart attachment example. Receipt of the first multipart attachment still needs confirmation from the clinic inbox after deployment.
