...
(function () {
  ...
  function build(lang, el) {
    ...
    notes.forEach(function (note) {
      var card = el('article', { class: 'card pn-card', 'data-note-id': note.id });
      if (note.demo) card.appendChild(caption(c.demo));
      var content = el('div', { class: 'pn-content' }, [note.image ? el('img', { src: note.image, alt: note.text, loading: 'lazy', width: '900', height: '1100' }) : el('div', { class: 'pn-text' }, [note.text])]);
      var surface = el('button', { type: 'button', class: 'pn-surface', 'aria-label': c.activate, 'aria-pressed': 'false', onclick: function () {
